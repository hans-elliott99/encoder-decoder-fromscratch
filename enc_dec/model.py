...
class TranslGRU:
    def __init__(self, vocab_size:int, 
                       ...
                       dec_type:str, 
                       stoi_mapping:dict, start_chr:str, stop_chr:str,
                ) -> None:
        """
        Define an Encoder-Decoder network for sequence to sequence translation.  

        Keyword arguments:\n
        vocab_size -- number of unique characters or words that appear in the data.\n
    ...
    def restore_weights(self, trained_weights, device):
        """
        Restore model weights by providing the list of trained weights saved in self.params.  
        self.init_weights() must be run first to initialize tensors of appropriate size and to specify the device.

        weights -- the list of weights saved in self.params, must be in the same order as when saved\n
        """
        ...
        assert len(trained_weights)==len(self.params)
        trained_weights = [p.to(self.device) for p in trained_weights]

        for rand_p, trained_p in zip(self.params, trained_weights):
            rand_p.data = trained_p.data

    def _init_enc_weights(self, g, device) -> None:
        """Initialize weight tensors for the encoder."""
        std = 1.0 / torch.sqrt(torch.tensor(self.dec_hidden_size)).item()
    ...
    def backprop_update(self, optimizer) -> None:
        ...
        # update
        # for i, p in enumerate(self.params): p.data += -lr * p.grad
        optimizer.step()
    ...
    def evaluate(self, english_text:str="hello", level="word"):
        ...
        if level=="word":
            ...
            for word in text.split():
                assert word in self.stoi, f"ensure all words are in the stoi_mapping ({word} is not)"
            SPACES = True
            ...
            text = english_text+"<"
            for c in text:
                assert c in self.stoi, f"ensure all characters are in the stoi_mapping. ({c} is not)"
            SPACES=False
